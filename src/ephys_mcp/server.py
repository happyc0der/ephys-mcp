...
def fit_decoder(
    ...
    _decoders[session_id] = {"model": model, "target": target, "bin_s": bin_s, "kind": kind}
    return {
        "kind": kind,
        "params": model.params,
        "target": target,
...
def analyze_session(session_id: str) -> str:
    ...
    return (
        ...
        "compare held-out R², and decode_window on a short test segment. Summarise findings and caveats."
    )
